...
signal orbs_changed(current_orbs: Array, orbs_added: Dictionary, orbs_removed: Dictionary)
...
#	print('remove_orbs ', current_orbs, ' ', orbs)
	...
	for orb in orbs:
		var count = orbs[orb]
		while count:
			current_orbs.remove_at(current_orbs.find(orb))
#			print('  removed ', orb)
			count -= 1
			orbs_removed += 1
	return orbs_removed
...
func should_use_ability(ability, attacker, target):
	if 'should_use_fn' in ability && ability.should_use_fn:
		var testfunc = ability.should_use_fn
		assert(testfunc.is_valid())
		return ability.should_use_fn.call(attacker, target)
	else:
		return true
